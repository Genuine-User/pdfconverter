...
<body class="bg-gray-50 text-gray-800">
  <div class="min-h-screen flex flex-col">
    ...
    <div id="loading-spinner" class="hidden fixed inset-0 bg-gray-500 bg-opacity-50 z-50">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      async function convertDocxToPdf(file) {
        ...
        // Convert DOCX to HTML with precise style mapping
        const { value: html, messages } = await mammoth.convertToHtml({
          arrayBuffer: arrayBuffer,
          transformDocument: mammoth.images.imgElement(function (image) {
        ...
        // Create temporary container with original document dimensions
        ...
        // Process images with original dimensions
        tempDiv.querySelectorAll('img').forEach(img => {
          // Convert pixel dimensions to physical units
          const pxToMm = (px) => px * 0.264583; // 96dpi to mm
          const originalWidth = img.naturalWidth || parseInt(img.width);
          const originalHeight = img.naturalHeight || parseInt(img.height);

          if (originalWidth && originalHeight) {
            img.style.width = pxToMm(originalWidth) + 'mm';
            img.style.height = pxToMm(originalHeight) + 'mm';
          }
          ...
          img.style.margin = '10mm auto';
        });

        // Process tables for professional layout
        ...
        try {
          ...
          // Add before html2canvas call
          tempDiv.querySelectorAll('table').forEach(table => {
            table.style.width = '100%';
            table.style.breakInside = 'avoid-page';
          });
          ...
          for (let el of elements) {
            const elHeight = el.offsetHeight * 0.264583; // Convert to mm
            if (currentHeight + elHeight > 297) { // A4 height
              pdf.addPage();
              currentHeight = 0;
            }
          ...
          // Add image with pixel-perfect dimensions
          pdf.addImage(imgData, 'PNG', 0, 0, imgWidth, imgHeight, undefined, 'FAST');

          updateProgress(80);

          // Add content with precise measurements
          let position = 0;
          ...
          pdf.save(file.name.replace(/\.docx?$/i, '_converted.pdf'));
          updateProgress(100);
        } catch (error) {
      ...
      async function convertPdfToDocx(file) {
        ...
        try {
          ...
          for (let i = 1; i <= pdf.numPages; i++) {
            ...
            // Create section for page content
            const section = {
              properties: {
                page: {
                  margin: {
            ...
            for (const item of content.items) {
              ...
              const isBold = item.fontName.includes("Bold");
              const fontSize = Math.round(item.height * 2);

              // Detect heading based on font size
              if (fontSize > 24) { // >12pt
                if (currentParagraph.length > 0) {
                  section.children.push(createParagraph(currentParagraph, currentStyle));
                  currentParagraph = [];
                }
                currentStyle = "Heading1";
              }
              ...
              const textRun = new docx.TextRun({
                ...
                bold: isBold,
                size: fontSize + "pt",
                font: item.fontName.split("-")[0] || "Arial"
              });
            ...
            updateProgress(30 + Math.round((i / pdf.numPages) * 50));
          }

          // Generate DOCX
          const blob = await docx.Packer.toBlob(doc);
          saveAs(blob, file.name.replace(/\.pdf$/i, '_converted.docx'));
          updateProgress(100);
        } catch (error) {
      ...
      function createParagraph(runs, style) {
        return new docx.Paragraph({
          ...
          indent: {
            left: style === "Normal" ? 400 : 0
          }
        });
      }

      // Helper functions
      function addStyledParagraph(runs, style, section) {
        section.addChildElement(new docx.Paragraph({
          children: runs,
          style: style,
          spacing: { after: 200 },
          indent: { left: 400 }
      ...
      async function extractPdfImages(opList, page) {
        const images = [];
        const svgGfx = new PDFJS.SVGGraphics(page.commonObjs, page.objs);
        await svgGfx.loadDependencies(opList);

        const svg = await svgGfx.getSVG(opList, page.view);
        ...
        images.push({
          data: await fetch(imageData).then(res => res.blob()),
          width: img.width,
          height: img.height
        });
